...
public class CommandBypassRemove implements Command<CommandSource> {
    ...
    public int run(CommandContext<CommandSource> context) throws CommandSyntaxException {
        ...
        if (!names.contains(name)) {
            context.getSource().sendSuccess(ITextComponent.nullToEmpty("\"" + name + "\" was not in the bypass list."), false);
        } else {
            names.remove(name);
            BypassConfig.bypassNames.set(String.join(",", names));
            WhiteList.reload();
            context.getSource().sendSuccess(ITextComponent.nullToEmpty("Removed \"" + name + "\" from the bypass list."), false);
        }
        return 0;
    }
}
